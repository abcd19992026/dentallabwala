...
export const doctorLedgerService = {
   ...
   */
  async getLabInfo(labId: string): Promise<{ lab_name: string; address?: string; studio_code?: string; mobile?: string; whatsapp_number?: string; logoUrl?: string } | null> {
    if (isSupabaseConfigured && labId) {
      try {
        const { data, error } = await supabase
          .from('labs')
          .select('lab_name, address, studio_code, mobile, whatsapp_number, logo_path')
          .eq('id', labId)
          .maybeSingle()
  ...
  async getSupplies(labId: string, doctorId: string): Promise<DoctorSupply[]> {
    ...
    try {
      ...
      return data.map((s) => ({
        ...
        tooth_no: s.tooth_no || '',
        per_unit_charge: Number(s.per_unit_charge) || 0,
  ...
  async addSupply(
    ...
    const newSupply: DoctorSupply = {
      ...
      per_unit_charge: Number(supplyData.per_unit_charge) || 0,
  ...
  async updateSupply(
    ...
    const merged: DoctorSupply = {
      ...
      per_unit_charge: Number(supplyData.per_unit_charge) || Number(existing?.per_unit_charge) || 0,
